...
// ESTRAZIONE PDF INTELLIGENTE (senza pattern hardcodati)
async function intelligentPdfExtraction(arrayBuffer: ArrayBuffer): Promise<string> {
  try {
    // Approccio 1: Prova con PDF.js se disponibile
    try {
      ...
      console.warn('PDF.js failed, using intelligent binary extraction:', pdfjsError)
      
      // Approccio 2: Estrazione binaria intelligente
      const uint8Array = new Uint8Array(arrayBuffer)
      const decoder = new TextDecoder('utf-8', { fatal: false })
      const rawText = decoder.decode(uint8Array)
      
      // Usa l'AI per identificare e estrarre testo significativo
      const meaningfulText = await extractMeaningfulTextWithAI(rawText)
